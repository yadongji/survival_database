...
-- Keep the database check constraint strict. The API payload may be incomplete
-- or may contain the old invalid zero value, so normalize only that field at
...
begin
    ...
    select p_account_id, coalesce(x.initial_wood, 0), coalesce(x.initial_gold, 0),
        ...
        coalesce(x.tower_final_damage_bonus_pct, 0), coalesce(x.tower_attack_per_second, 0),
        coalesce(x.wall_damage_block, 0), coalesce(x.wall_damage_reduction_pct, 0),
        coalesce(x.wall_armor_bonus_pct, 0), coalesce(x.wall_health_regen_per_second, 0),
        coalesce(x.wall_initial_health, 0), coalesce(x.wall_armor, 0),
        coalesce(x.wall_health_bonus_pct, 0), coalesce(x.wall_health_per_second, 0),
        coalesce(x.lumberjack_attack_growth, 0), coalesce(x.lumberjack_attack_speed_bonus_pct, 0),
    ...
    from jsonb_to_record(p_gameplay_stats) as x(
        ...
        hero_health_bonus_pct numeric, hero_damage_reduction_pct numeric,
        ...
        tower_attack_per_second numeric, wall_damage_block numeric,
        wall_damage_reduction_pct numeric, wall_armor_bonus_pct numeric,
        wall_health_regen_per_second numeric, wall_initial_health bigint,
        wall_armor numeric, wall_health_bonus_pct numeric,
